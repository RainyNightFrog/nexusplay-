...
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell){--void-side-nav:102px;--void-mobile-nav-h:0px}
...
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app{display:block!important;visibility:visible!important;height:auto!important;max-height:none!important;min-height:100%!important;overflow:visible!important;padding-top:var(--void-app-header-offset,clamp(108px,13vh,148px))!important;padding-bottom:max(24px,env(safe-area-inset-bottom,0px))!important;padding-left:calc(var(--void-side-nav,102px) + 20px)!important;padding-right:calc(var(--void-side-nav,102px) + 20px)!important}
...
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app>.app-header{position:fixed!important;top:0!important;left:0!important;width:100%!important;z-index:999;display:grid!important;grid-template-columns:auto minmax(0,1fr) auto!important;align-items:center!important;gap:10px 14px!important;padding:10px calc(var(--void-side-nav,102px) + 20px)!important;box-sizing:border-box!important;visibility:visible!important}
...
function applyVoidEmbedBoot(html: string) {
  ...
  let out = html;
  /* 一律以平台最新 boot 覆寫，確保閘道／手機自動放大修復無需重傳 zip */
  if (bootRe.test(out)) {
    out = out.replace(bootRe, VOID_EMBED_BOOT_SCRIPT);
  } else if (out.includes("<head>")) {
    out = out.replace("<head>", `<head>${VOID_EMBED_BOOT_SCRIPT}`);
  } else {
    out = out.replace(/<head[^>]*>/i, (match) => `${match}${VOID_EMBED_BOOT_SCRIPT}`);
  }
  return out;
}
