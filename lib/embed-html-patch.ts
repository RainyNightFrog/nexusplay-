...
/* 版面與 void-gacha.com 一致：左右對稱 side-nav + 20px（右欄佔右側留白），標題／跑馬燈置中 */
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell),
html.void-itch-embed.void-itch-expanded:not(.void-itch-mobile-shell){--void-side-nav:102px;--void-mobile-nav-h:0px;--void-app-header-offset:clamp(118px,12vh,148px)}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell),html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) body{overflow-x:hidden!important;overflow-y:auto!important;height:auto!important;min-height:100%!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app,
html.void-itch-embed:not(.void-itch-mobile-shell).void-itch-expanded #screen-app{display:block!important;visibility:visible!important;height:auto!important;max-height:none!important;min-height:100%!important;overflow:visible!important;padding-top:var(--void-app-header-offset,clamp(118px,12vh,148px))!important;padding-bottom:max(24px,env(safe-area-inset-bottom,0px))!important;padding-left:calc(var(--void-side-nav,102px) + 20px)!important;padding-right:calc(var(--void-side-nav,102px) + 20px)!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) .void-itch-nav-dock{display:none!important;visibility:hidden!important;pointer-events:none!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app>.side-nav{position:fixed!important;top:0!important;right:0!important;bottom:0!important;left:auto!important;width:var(--void-side-nav,102px)!important;max-width:none!important;min-width:0!important;flex-direction:column!important;overflow:hidden!important;z-index:120!important;display:flex!important;visibility:visible!important;opacity:1!important;pointer-events:auto!important;height:auto!important;max-height:none!important;padding:var(--void-app-header-offset,clamp(118px,12vh,148px)) 8px max(6px,env(safe-area-inset-bottom,0px))!important;border-radius:0 0 0 16px!important;transform:none!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app>.side-nav .nav-btn{flex:1 1 0!important;width:100%!important;min-width:0!important;max-width:none!important;min-height:0!important;padding:3px 4px!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app>.side-nav .nav-icon{font-size:clamp(17px,2.1vh,22px)!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app>.side-nav .nav-label{font-size:clamp(10px,1.05vh,11px)!important;line-height:1.15!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) #screen-app>.app-header{position:fixed!important;top:0!important;left:0!important;width:100%!important;z-index:999;display:grid!important;grid-template-columns:auto minmax(0,1fr) auto!important;align-items:center!important;gap:10px 14px!important;padding:10px calc(var(--void-side-nav,102px) + 20px) 10px calc(var(--void-side-nav,102px) + 20px)!important;box-sizing:border-box!important;visibility:visible!important}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) .home-left-pillar{left:8px;width:min(300px,24vw)}
html.void-rainynightfrog-embed.np-embed-expanded:not(.void-itch-mobile-shell) .home-right-rail{right:max(12px,calc(var(--void-side-nav,102px) * .12));width:min(240px,22vw)}
/* 卡牌尺寸：補上 np-embed-expanded，對齊 itch void-itch-expanded（主畫面／背包等） */
html.void-itch-embed.np-embed-expanded .home-arena .card-3d-wrap,
html.void-itch-embed.np-embed-expanded .mythic-rising-wall .card-3d-wrap,
html.void-itch-embed.np-embed-expanded .bag-card-mount .card-3d-wrap{aspect-ratio:var(--void-card-aspect,2 / 3);width:auto;height:auto;max-height:min(46dvh,400px);max-width:min(36vw,280px)}
html.void-itch-embed.np-embed-expanded .showcase-slot .card-3d-wrap{width:100%!important;height:100%!important;max-width:100%!important;max-height:100%!important}
html.void-itch-embed.np-embed-expanded .epic-hero-card{aspect-ratio:5 / 7;width:min(var(--epic-card-w,168px),32vw,240px);height:auto;max-height:min(44dvh,360px)}
html.void-itch-embed.np-embed-expanded .home-stage--browse{width:100%;max-width:min(1200px,100%);margin-inline:auto}
html.void-rainynightfrog-embed .void-itch-footer-toggle{display:none!important}
html.void-rainynightfrog-embed .void-global-footer.void-itch-footer-collapsed .footer-content,html.void-rainynightfrog-embed .void-global-footer .footer-content{display:block!important}
</style>`;

/** 同步 itch 展開 class，讓 itch-embed.css 卡牌／版面規則生效（對齊 void-gacha.com） */
const VOID_ITCH_EXPAND_SYNC_SCRIPT = `<script id="void-itch-expand-sync">(function(){
  var d=document.documentElement;
  function sync(){
    var expanded=d.classList.contains('np-embed-expanded');
    var compact=d.classList.contains('np-embed-compact')&&!expanded;
    d.classList.toggle('void-itch-expanded',!!expanded);
    d.classList.toggle('void-itch-compact',!!compact);
  }
  sync();
  try{
...
function applyVoidEmbedBoot(html: string) {
  ...
  if (bootRe.test(html)) {
    /* zip 已含 boot：保留壓縮檔版本，避免平台常數覆寫導致你的修正遺失 */
    return html;
  }
  if (html.includes("<head>")) {
...
function applyVoidScrollFix(html: string) {
  ...
  const syncTag = VOID_ITCH_EXPAND_SYNC_SCRIPT.trim();
  const styleRe = /<style id="rainynightfrog-scroll-fix">[\s\S]*?<\/style>/i;
  const syncRe = /<script id="void-itch-expand-sync">[\s\S]*?<\/script>/i;
  let out = html;
  if (styleRe.test(out)) {
    out = out.replace(styleRe, "");
  }
  if (syncRe.test(out)) {
    out = out.replace(syncRe, "");
  }
  const inject = `${styleTag}${syncTag}`;
  /* 必須插在 </head> 前（全部 CSS 之後），否則 zip bundle 會蓋掉閘道規則 */
  if (/<\/head>/i.test(out)) {
    return out.replace(/<\/head>/i, `${inject}</head>`);
  }
  if (out.includes("<head>")) {
    return out.replace("<head>", `<head>${inject}`);
  }
  return `${inject}${out}`;
}
