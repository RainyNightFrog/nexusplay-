...
async function main() {
  ...
  try {
    const sql = readFileSync(
      resolve(process.cwd(), "supabase/achievements-titles-expand.sql"),
      "utf8"
    );
    await client.query(sql);
    const { rows } = await client.query("select count(*)::int as n from public.titles");
    console.log(`✓ 稱號擴充完成，共 ${rows[0].n} 個可佩戴稱號`);
  } finally {
    await client.end();
  }
}
