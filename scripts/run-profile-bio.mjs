...
const SYNC_METADATA_BIO_SQL = `
update public.profiles p
set bio = left(nullif(trim(u.raw_user_meta_data->>'bio'), ''), 300)
from auth.users u
where p.id = u.id
  and p.bio is null
  and nullif(trim(u.raw_user_meta_data->>'bio'), '') is not null;
`;
...
async function main() {
  ...
  try {
    const sql = readFileSync(
      resolve(process.cwd(), "supabase/profile-bio.sql"),
      "utf8"
    );
    await client.query(sql);
    console.log("✓ profiles.bio 欄位已就緒");

    const syncResult = await client.query(SYNC_METADATA_BIO_SQL);
    console.log(`✓ 已從 user_metadata 同步 ${syncResult.rowCount ?? 0} 筆自我介紹`);
  } finally {
    await client.end();
  }
}
