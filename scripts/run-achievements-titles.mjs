...
async function main() {
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/achievements-titles.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/achievements-titles.sql …");
    await client.query(sql);
    console.log("✓ 成就與稱號系統已就緒");

    const { rows: tables } = await client.query(`
      select table_name
      from information_schema.tables
      where table_schema = 'public'
        and table_name in ('achievements', 'titles', 'user_achievements', 'user_titles')
      order by table_name;
    `);
    ...
    const { rows: achievements } = await client.query(
      "select code, title, points from public.achievements order by points"
    );
    console.log("\n範例成就：");
    for (const row of achievements) {
      console.log(`  - [${row.code}] ${row.title} (+${row.points} AP)`);
    }
  } finally {
    await client.end();
  }
