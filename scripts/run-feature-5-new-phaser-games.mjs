...
async function main() {
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/feature-5-new-phaser-games.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/feature-5-new-phaser-games.sql …");
    await client.query(sql);
    console.log("✓ 5 款新 Phaser 3 遊戲 featured 狀態已更新");

    const { rows } = await client.query(`
      select slug, is_featured, featured_sort, featured_badge
      from public.games
      where slug in (
        'cyber-blade-dash',
        'neon-pinball-frenzy',
      ...
      order by featured_sort desc, slug asc;
    `);

    if (rows.length === 0) {
      console.warn(
        "\n⚠ 找不到任何目標 slug。請先確認 5 款新遊戲已經 seed 到 public.games。"
      );
    } else {
      console.log("\n目前 featured 設定：");
      for (const row of rows) {
        console.log(
          `  - ${row.slug}: featured=${row.is_featured} sort=${row.featured_sort} badge=${row.featured_badge ?? "null"}`
        );
      }
    }
  } finally {
