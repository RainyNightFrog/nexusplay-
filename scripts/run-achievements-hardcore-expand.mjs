...
async function main() {
  ...
  try {
    const sql = readFileSync(
      resolve(process.cwd(), "supabase/achievements-hardcore-expand.sql"),
      "utf8"
    );
    ...
    const { rows: titleRows } = await client.query(
      "select count(*)::int as n from public.titles"
    );
    console.log(
      `✓ 高難度成就／特別稱號套用完成：成就 ${achievementRows[0].n}、稱號 ${titleRows[0].n}`
    );
  } finally {
    await client.end();
  }
