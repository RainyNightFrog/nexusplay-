...
async function main() {
  ...
  if (candidates.length === 0) {
    ...
    console.error(
      "密碼位置：Supabase → Settings → Database → Database password"
    );
    process.exit(1);
  }
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/add-game-stats.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/add-game-stats.sql …");
    await client.query(sql);
    console.log("✓ 欄位 plays_count、rating_avg 與索引已就緒");
    ...
    const { rows } = await client.query(`
      ...
      where table_schema = 'public'
        ...
        and column_name in ('plays_count', 'rating_avg')
