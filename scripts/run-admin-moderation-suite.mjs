...
async function main() {
  ...
  if (candidates.length === 0) {
    console.error("缺少資料庫連線資訊。請在 .env.local 加入 SUPABASE_DB_PASSWORD");
    process.exit(1);
  }
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/admin-moderation-suite.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/admin-moderation-suite.sql …");
    await client.query(sql);
    console.log("✓ 超級管理員 moderation 擴充架構已就緒");
  } finally {
    await client.end();
  }
}
