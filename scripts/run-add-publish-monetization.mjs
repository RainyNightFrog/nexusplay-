...
async function main() {
  ...
  try {
    const sqlPath = resolve(
      process.cwd(),
      "supabase/add-publish-monetization.sql"
    );
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/add-publish-monetization.sql …");
    await client.query(sql);
    console.log("✓ publish_status、tips_enabled、suggested_tip_amount 已就緒");
    ...
    const { rows } = await client.query(`
      ...
      where table_schema = 'public'
        ...
        and column_name in ('publish_status', 'tips_enabled', 'suggested_tip_amount')
