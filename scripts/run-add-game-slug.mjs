...
async function main() {
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/add-game-slug.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/add-game-slug.sql …");
    await client.query(sql);
    console.log("✓ games.slug 欄位、唯一索引與格式檢查已就緒");
    ...
    if (slugRows[0]) {
      console.log(
        `\n已有 slug 的遊戲：${slugRows[0].with_slug} / ${slugRows[0].total}`
      );
    }
  } finally {
    await client.end();
