...
async function main() {
  ...
  if (candidates.length === 0) {
    console.error("缺少 SUPABASE_DB_PASSWORD，無法執行 forum.sql");
    process.exit(1);
  }
  ...
  try {
    const sql = readFileSync(resolve(process.cwd(), "supabase/forum.sql"), "utf8");
    console.log("執行 supabase/forum.sql …");
    await client.query(sql);
    console.log("✓ forum_posts / forum_comments 已就緒");
  } finally {
    await client.end();
  }
}
