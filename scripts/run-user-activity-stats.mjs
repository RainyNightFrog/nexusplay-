...
async function main() {
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/user-activity-stats.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/user-activity-stats.sql …");
    await client.query(sql);
    console.log("✓ 全站活躍度統計表已就緒");

    const { rows: columns } = await client.query(`
      select column_name, data_type
      from information_schema.columns
      where table_schema = 'public'
        and table_name = 'user_activity_stats'
      order by ordinal_position;
    `);

    console.log("\nuser_activity_stats 欄位：");
