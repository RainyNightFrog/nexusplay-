...
async function main() {
  ...
  if (candidates.length === 0) {
    console.error("缺少 SUPABASE_DB_PASSWORD，無法執行 game-page-content.sql");
    process.exit(1);
  }
  ...
  try {
    const sql = readFileSync(
      resolve(process.cwd(), "supabase/game-page-content.sql"),
      "utf8"
    );
    console.log("執行 supabase/game-page-content.sql …");
    await client.query(sql);
    console.log("✓ gallery_urls / devlog_entries / game_comments 已就緒");
  } finally {
    await client.end();
  }
}
