...
async function main() {
  ...
  if (candidates.length === 0) {
    console.error("缺少 SUPABASE_DB_PASSWORD，無法執行 chat-messages.sql");
    process.exit(1);
  }
  ...
  try {
    const sql = readFileSync(
      resolve(process.cwd(), "supabase/chat-messages.sql"),
      "utf8"
    );
    console.log("執行 supabase/chat-messages.sql …");
    await client.query(sql);
    console.log("✓ chat_messages 已就緒");
  } finally {
    await client.end();
  }
}
