...
async function main() {
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/game-legacy-imports.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/game-legacy-imports.sql …");
    await client.query(sql);
    console.log("✓ game_legacy_imports 表與 RLS 已就緒");
  } finally {
    await client.end();
  }
}
