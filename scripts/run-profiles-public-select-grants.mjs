...
async function main() {
  ...
  try {
    const sqlPath = resolve(
      process.cwd(),
      "supabase/profiles-public-select-grants-2026.sql"
    );
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 profiles-public-select-grants-2026.sql …");
    await client.query(sql);
    console.log("✓ profiles 公開可讀欄位授權已更新");
    ...
    console.log("\n目前 anon / authenticated 欄位權限：");
    for (const row of rows) {
      console.log(`  - ${row.grantee} ${row.privilege_type}: ${row.columns}`);
    }
  } finally {
    await client.end();
  }
