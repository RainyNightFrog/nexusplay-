...
async function main() {
  ...
  try {
    const sql = readFileSync(resolve(process.cwd(), "supabase/achievements-expand.sql"), "utf8");
    await client.query(sql);
    const { rows } = await client.query("select count(*)::int as n from public.achievements");
    console.log(`✓ 成就擴充完成，共 ${rows[0].n} 項成就`);
  } finally {
    await client.end();
  }
}
