...
async function main() {
  ...
  if (candidates.length === 0) {
    console.error("缺少 SUPABASE_DB_PASSWORD，無法執行 game-publish-metadata.sql");
    process.exit(1);
  }
  ...
  try {
    const sql = readFileSync(
      resolve(process.cwd(), "supabase/game-publish-metadata.sql"),
      "utf8"
    );
    console.log("執行 supabase/game-publish-metadata.sql …");
    await client.query(sql);
    console.log("✓ tags / viewport / ai_disclosed / details_html 欄位已就緒");
  } finally {
    await client.end();
  }
}
