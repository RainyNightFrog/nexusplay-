...
async function main() {
  ...
  try {
    const sqlPath = resolve(
      process.cwd(),
      "supabase/add-stripe-user-connect-fields.sql"
    );
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/add-stripe-user-connect-fields.sql …");
    await client.query(sql);
    console.log("✓ stripe_account_id、stripe_details_submitted 已就緒");
    ...
    const { rows } = await client.query(`
      ...
      where table_schema = 'public'
        ...
        and column_name in ('stripe_account_id', 'stripe_details_submitted')
