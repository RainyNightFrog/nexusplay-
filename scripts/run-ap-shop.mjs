...
async function main() {
  ...
  try {
    const sql = readFileSync(resolve(process.cwd(), "supabase/ap-shop.sql"), "utf8");
    await client.query(sql);
    const { rows: items } = await client.query(
      "select count(*)::int as n from public.ap_shop_items where active"
    );
    const { rows: wallets } = await client.query(
      "select count(*)::int as n from public.user_ap_wallet"
    );
    console.log(
      `✓ AP 商店完成：商品 ${items[0].n}、已建立錢包 ${wallets[0].n}`
    );
  } finally {
    await client.end();
  }
