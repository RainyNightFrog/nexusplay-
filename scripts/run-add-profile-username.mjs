...
async function main() {
  ...
  try {
    const sqlPath = resolve(process.cwd(), "supabase/add-profile-username.sql");
    const sql = readFileSync(sqlPath, "utf8");
    console.log("執行 supabase/add-profile-username.sql …");
    await client.query(sql);
    console.log("✓ profiles.username 欄位、唯一索引與格式檢查已就緒");
