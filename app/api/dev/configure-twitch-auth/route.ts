...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!patchUrlsOnly && (!clientId || !clientSecret)) {
      return NextResponse.json(
        { error: "請填寫 Twitch Client ID 與 Client Secret" },
        { status: 400 }
      );
    }
    ...
    if (!patchUrlsOnly) {
      patchBody.external_twitch_enabled = true;
      patchBody.external_twitch_client_id = clientId;
      patchBody.external_twitch_secret = clientSecret;
    }
