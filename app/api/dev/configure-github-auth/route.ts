...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!patchUrlsOnly && (!clientId || !clientSecret)) {
      return NextResponse.json(
        { error: "請填寫 GitHub Client ID 與 Client Secret" },
        { status: 400 }
      );
    }
    ...
    if (!patchUrlsOnly) {
      patchBody.external_github_enabled = true;
      patchBody.external_github_client_id = clientId;
      patchBody.external_github_secret = clientSecret;
    }
