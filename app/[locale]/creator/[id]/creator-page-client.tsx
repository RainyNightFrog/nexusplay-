...
type CreatorPageClientProps = {
  initialCreator?: PublicCreatorProfile | null;
  creatorId: string;
};
...
export default function CreatorPageClient({
  ...
  const [creator, setCreator] = useState<PublicCreatorProfile | null>(
    initialCreator
  );
  ...
  useEffect(() => {
    ...
    fetch(`/api/creators/${creatorId}`)
      ...
      .catch(() => {
        if (!cancelled && !initialCreator) setError(t("loadFailed"));
      })
      .finally(() => {
