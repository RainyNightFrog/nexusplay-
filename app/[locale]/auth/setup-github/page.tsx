...
const GITHUB_REDIRECT_URI = `https://${PROJECT_REF}.supabase.co/auth/v1/callback`;

const LINKS = {
  githubNewApp: "https://github.com/settings/applications/new",
  supabaseToken: "https://supabase.com/dashboard/account/tokens",
  supabaseGithub: `https://supabase.com/dashboard/project/${PROJECT_REF}/auth/providers?provider=GitHub`,
};
...
export default function SetupGithubPage() {
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      const response = await fetch("/api/dev/configure-github-auth", {
        method: "POST",
  ...
  return (
    <div className="dark mx-auto min-h-full max-w-xl px-4 py-10 text-zinc-100">
      <div className="mb-8 space-y-2 text-center">
        <h1 className="text-2xl font-bold text-white">GitHub 登入一鍵設定</h1>
        <p className="text-sm text-zinc-400">
          GitHub 後台需你本人登入一次；貼上 3 個值後，其餘由系統自動完成。
        </p>
      </div>

      <ol className="mb-8 space-y-4 text-sm text-zinc-300">
        <li className="rounded-xl border border-white/10 bg-white/5 p-4">
          <p className="font-medium text-white">① GitHub 建立 OAuth App</p>
          <p className="mt-1 text-zinc-400">
            Application name → RainyNightFrog · Homepage → {PRODUCTION_SITE_URL}
          </p>
          <p className="mt-1 text-zinc-400">Authorization callback URL 貼下面這行：</p>
          ...
          <Button
            ...
            onClick={async () => {
              await copyText(GITHUB_REDIRECT_URI);
              setCopied(true);
              window.setTimeout(() => setCopied(false), 2000);
            }}
          >
            <Copy className="size-3.5" />
            {copied ? "已複製" : "複製 Callback URL"}
          </Button>
          <a
          ...
          >
            新建 GitHub OAuth App <ExternalLink className="size-3.5" />
          </a>
        </li>
        ...
        <li className="rounded-xl border border-white/10 bg-white/5 p-4">
          ...
          <p className="mt-1 text-zinc-400">
            在 OAuth App 設定頁取得；Secret 只顯示一次，請立即複製
          </p>
        </li>
        ...
        <li className="rounded-xl border border-white/10 bg-white/5 p-4">
          ...
          <a
            href={LINKS.supabaseGithub}
            target="_blank"
            rel="noopener noreferrer"
            className="ml-3 inline-flex items-center gap-1 text-cyan-400 hover:underline"
          >
            Supabase GitHub Provider <ExternalLink className="size-3.5" />
          </a>
        </li>
      </ol>
      ...
      {done ? (
        <div className="rounded-2xl border border-emerald-400/25 bg-emerald-500/10 p-6 text-center">
          ...
          <p className="font-semibold text-white">GitHub 登入已設定完成！</p>
          <p className="mt-2 text-sm text-zinc-400">
            前往登入頁測試「使用 GitHub 登入」
          </p>
        ...
        >
          ...
          <label className="flex items-center gap-2 text-sm text-zinc-300">
            ...
            />
            僅更新正式站 Auth URL（不修改 GitHub Provider）
          </label>
